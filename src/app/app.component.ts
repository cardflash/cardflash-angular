...
export class AppComponent {
  ...
  //cards
  public cards: Card[] = [
    { id: this.nanoid(), front: '', back: '', page: this.page, hiddenText: '', chapter: '', title: '' },
  ];
  ...
  async addSelection(text: string = '') {
    ...
    let saveCardId: string = this.cards[this.currIndex].id;
    ...
    if (this.config.addTextOption && this.config.addOcrTextOption) {
      ...
      recognize(dataURL, this.config.useEnglishOcr ? 'eng' : 'deu', {
        ...
        let card = this.cards.find((c) => c.id === saveCardId);
        if(card){
        if (this.config.addTextAsHidden) {
  ...
  deleteCard(id: string) {
    let i = this.cards.findIndex((c,i) => c.id === id);
    if(i >= 0){
        if (i === 0) {
